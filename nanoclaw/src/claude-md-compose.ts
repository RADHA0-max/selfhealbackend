...
function syncSymlink(linkPath: string, target: string): void {
  ...
  try {
    ...
    if (err?.code === 'EPERM' && process.platform === 'win32') {
      ...
      // Write a stub file — the target is a container path (dangling on host
      // anyway), so a placeholder is functionally equivalent for host-side
      // compose and the real content is RO-mounted inside the container.
      fs.writeFileSync(linkPath, `<!-- symlink stub: ${target} -->\n`);
    } else {
      throw err;
    }
  }
